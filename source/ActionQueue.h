...
public:
	...
	void printEachDamage(ActionSlot& slot) {
		...
		for (size_t i = 0; i < slot.selecting.coin_list.size(); i++)
		{
			if (!slot.selecting.coin_list[i].is_Broke || slot.selecting.coin_list[i].type == "Unbreakable")  // 未破碎的普通硬币
			{
				...
				//{
				//	if (!slot.selecting.coin_list[i + 1].is_Broke || slot.selecting.coin_list[i + 1].type == "Unbreakable") {
				//		std::cout << "+";
				//	}
				//}
			}
	...
	void makeDamage(ActionSlot& attacker, ActionSlot& target) {
		...
		// 计算等压
		float diff = 0.0f;
		if (attacker.selecting.attack_level > target.selecting.attack_level) {
			diff = attacker.selecting.attack_level - target.Owner->Data->level;
		}
		// 增伤
		...
		for (size_t i = 0; i < attacker.selecting.coin_list.size(); i++)
		{
			...
			if (!current->is_Broke || current->type == "Unbreakable")
			{
				...
				setColor(8);
				// 攻防差值
				float level_diff_enhance = floorf((1 + diff / (abs(diff) + 25)) * 10.0f) / 10.0f;
				if ((int)(level_diff_enhance * 100) - 100 > 0) {
					std::cout << "[日志] 等级压制增伤：";
					setColor(15);
					std::cout << (int)(level_diff_enhance * 100) << "%\n";
					setColor(8);
					mul_1 += level_diff_enhance;
				}
				...
				{
					attacker.Owner->rounder_enhance = 1;
				...
				{
					target.Owner->rounder_protect = 1;
					float protect = min(target.Owner->protect.x / 10.0f, 1.0f);
					mul_2 -= protect;
				}
				// 易损
				if (target.Owner->fragile.x > 0.0f)
				{
					target.Owner->rounder_fragile = 1;
					float fragile = min(target.Owner->fragile.x / 10.0f, 1.0f);
					mul_2 += fragile;
				}

				float total_damage_enhance = (mul_1 * mul_2 + add_1 + add_2);
				std::cout << "[日志] 总增伤：";
				setColor(15);
				std::cout << "x" << (int)(total_damage_enhance * 100) << "%\n";
				setColor(8);

				current->damage *= total_damage_enhance;
