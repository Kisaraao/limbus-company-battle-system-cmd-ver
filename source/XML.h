...
public:
	void loadXML(const std::string& path, CharacterTemplate* ch) {
		...
		// 客制化充能
		ch->custom_charge.enable = node.child("CustomCharge").attribute("enable").as_bool(false);
		ch->custom_charge.name = node.child("CustomCharge").attribute("name").as_string("充能");
		ch->custom_charge.color = node.child("CustomCharge").attribute("color").as_int(15);
		ch->custom_charge.double_count = node.child("CustomCharge").attribute("double_count").as_bool(true);
		// 开局自带特效
		for (auto& ptr : node.child("OnGameStart").children("Effect"))
		{
			CoinEffect effect;
			effect.trigger = ptr.attribute("trigger").as_string();
			...
			ch->on_game_start_effect_list.push_back(effect);
		}
		// 技能列表
		for (pugi::xml_node& skl : node.children("Skill"))
		{
			...
			{
				skill.using_speak = skl.child("Using").child("Speak").attribute("content").as_string();

				// change skill
				for (auto& ptr : skl.child("Using").children("Change"))
				{
					SkillChange change;
					change.notice = ptr.attribute("notice").as_bool();
					change.effect = ptr.attribute("effect").as_string();
					change.target = ptr.attribute("target").as_string();
					change.mode = ptr.attribute("mode").as_string();
					change.vec = ptr.attribute("vec").as_string();
					change.value = ptr.attribute("value").as_int();
					change.skill = ptr.attribute("skill").as_int();
					skill.change_list.push_back(change);
				}

				for (auto& ptr : skl.child("Using").children("Enhance"))
				{
					SkillEnhance enhance;
					enhance.notice = ptr.attribute("notice").as_bool();
					enhance.compare_target = ptr.attribute("compare_target").as_string();
					enhance.compare_effect = ptr.attribute("compare_effect").as_string();
					enhance.mode = ptr.attribute("mode").as_string();
					enhance.compare_vec = ptr.attribute("compare_vec").as_string();
					enhance.compare_value = ptr.attribute("compare_value").as_int();

					enhance.enhance_effect = ptr.attribute("enhance_effect").as_string();
					enhance.enhance_vec = ptr.attribute("enhance_vec").as_string();
					enhance.enhance_value = ptr.attribute("enhance_value").as_int();
					
					enhance.has_cost = ptr.attribute("has_cost").as_bool();
					enhance.cost_effect = ptr.attribute("cost_effect").as_string();
					enhance.cost_vec = ptr.attribute("cost_vec").as_string();
					enhance.cost_value = ptr.attribute("cost_value").as_int();

					skill.enhance_list.push_back(enhance);
				}
			...
			if (!skl.child("Combat").empty())
			{
				...
				{
					skill.combat_win_speak = skl.child("Combat").child("Win").child("Speak").attribute("content").as_string();
					for (auto& ptr : skl.child("Combat").child("Win").children("Effect"))
					{
						CoinEffect win;
						win.trigger = ptr.attribute("trigger").as_string();
				...
				{
					skill.combat_lose_speak = skl.child("Combat").child("Lose").child("Speak").attribute("content").as_string();
					for (auto& ptr : skl.child("Combat").child("Lose").children("Effect"))
					{
						CoinEffect lose;
						lose.trigger = ptr.attribute("trigger").as_string();
